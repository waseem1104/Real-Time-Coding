...
export default function PrivateChat({requestUser}) {
    ...
    useEffect(() => {
        ...
        if (requestUser) {
            ...
            request.open("GET", `http://localhost:5000/admin/privateMessages/${requestUser[0].userid}`, false);
        ...
        request.onreadystatechange = function () {
            if (request.readyState == XMLHttpRequest.DONE) {
                ...
                socket.emit('private message', {
                    ...
                    to: JSON.parse(request.responseText).userid
                });
            }
        }
        request.open("POST", `http://localhost:5000/admin/privateMessages/new`, false);
        request.setRequestHeader("Content-type", "application/json");
        request.setRequestHeader('Authorization', "Bearer " + cookies.get('token'));
        request.send(JSON.stringify({
            "content": message,
            "userid": requestUser[0].userid
        }));
